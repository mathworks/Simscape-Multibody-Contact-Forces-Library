...
CF_bpth_pla2pla =getfullname(Simulink.findBlocks(mdlname,'force_law_pla2pla','.*',f));
...
if(~isempty(CF_bpth))
    ...
    for i=1:size(CF_bpth,1)
        ...
        if(within_composite_force==0)
            set_param(CF_bpth{i},'force_law',contactModel);
...
if(~isempty(CF_bpth_box2box))
    for j=1:size(CF_bpth_box2box,1)
        set_param(CF_bpth_box2box{j},'force_law_box2box',contactModel)
    end
end

if(~isempty(CF_bpth_box2belt))
    for j=1:size(CF_bpth_box2belt,1)
        set_param(CF_bpth_box2belt{j},'force_law_box2belt',contactModel)
    end
end

if(~isempty(CF_bpth_sph2belt))
    for j=1:size(CF_bpth_sph2belt,1)
        set_param(CF_bpth_sph2belt{j},'force_law_sph2bel',contactModel)
    end
end

if(~isempty(CF_bpth_pla2pla))
    for j=1:size(CF_bpth_pla2pla,1)
        set_param(CF_bpth_pla2pla{j},'friction_law_pla2pla',contactModel)
    end
end
